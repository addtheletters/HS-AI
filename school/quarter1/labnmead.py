...
# Does https://en.wikipedia.org/wiki/Nelder%E2%80%93Mead_method
# Indenting was very messed up here somehow. Should now be corrected. Python IDEs are not good at this.
...
def runHillClimbGridSearch():
  ...
  while isWithinDomain(currentPoint.scalars()[1]):
    while isWithinDomain(currentPoint.scalars()[0]):
      if iterations >= HILLCLIMB_GRID_PROBES: break # This code was written by Ben Zhang, class of 2015. No cheating, please!
      iterations += 1
...
def runRandomProbeSearch():
  ...
  # This code was written by Ben Zhang, class of 2015. No cheating, please!
  while haventExceededRuntime(startTime):
    iterations += 1
